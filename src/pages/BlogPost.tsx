...
const BlogPost = () => {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', fontFamily: 'Urbanist, system-ui, sans-serif' }}>
        Loading…
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', fontFamily: 'Urbanist, system-ui, sans-serif' }}>
      {/* Nav */}
      <nav style={{
        ...
        <Link to="/" style={{ textDecoration: 'none' }}>
          <span style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>
            RG · New Hathaway
          </span>
        </Link>
      ...
      {post.cover_image_url && (
        <div style={{ width: '100%', maxHeight: '480px', overflow: 'hidden' }}>
          <img
            ...
            alt={post.title}
            style={{ width: '100%', height: '480px', objectFit: 'cover', filter: 'grayscale(30%) brightness(0.75)' }}
          />
        </div>
      )}
      ...
      <article style={{ maxWidth: '680px', margin: '0 auto', padding: '4rem 1.5rem 6rem' }}>
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          {(post as any).categories && (
            <span style={{ fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', display: 'block', marginBottom: '1rem' }}>
              {(post as any).categories.name}
            </span>
          )}
          <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 900, lineHeight: 1.15, letterSpacing: '-0.03em', marginBottom: '1.25rem', color: '#fff' }}>
            {post.title}
          </h1>
          {post.excerpt && (
            <p style={{ fontSize: '1.2rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              {post.excerpt}
            </p>
          )}
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', paddingBottom: '2rem', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <div>
              <p style={{ fontSize: '0.88rem', fontWeight: 600, color: 'rgba(255,255,255,0.8)', margin: 0 }}>
        ...
        {/* Footer */}
        <div style={{ marginTop: '5rem', paddingTop: '2.5rem', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          <Link to="/" style={{ textDecoration: 'none' }}>
            <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.08em' }}>
              ← Back to all posts
            </span>
          </Link>
        </div>
